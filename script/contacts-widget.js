Array.from(document.querySelectorAll('.contacts-widget')).forEach(widget => {
    const openButton = widget.querySelector('.contacts-widget__open');
    const content = widget.querySelector('.contacts-widget__content');

    if (!openButton || !content) return

    openButton.addEventListener('click', () => {
        widget.classList.toggle('contacts-widget_is-open');
    });

    document.addEventListener('click', (event) => {
        if (!widget.contains(event.target)) {
            widget.classList.remove('contacts-widget_is-open');
        }
    });
